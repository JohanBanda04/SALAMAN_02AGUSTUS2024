...
class DatapinjamanController extends Controller
{
    ...
    {
        //return "data berita satker broyyy";
        /*penggunaan define gate*/
        //$this->authorize('admin');
        $dtSatkerCek = DB::table('satker')->get();
        /*pembatasan membatasi akses data tiap role*/
...
//        echo count($url_gabung);
    ...
    public function formpinjamkendaraan()
    {
        $subbidangs = DB::table('subbidang')->get();
        $kendaraans = DB::table('kendaraan')->get();
        return view('pinjamsarpras.formpinjamkendaraan', compact('subbidangs', 'kendaraans'));
    }
    ...
    public function storepinjamkendaraan(Request $request)
    {
        ...
        $perihal_pinjaman = $request->perihal_pinjaman;
        $mulai_tgl = $request->mulai_tgl;
        $sampai_tgl = $request->sampai_tgl;
        $penanggung_jawab = $request->penanggung_jawab;
        $subbidang_kode = $request->subbidang_kode;
        $kode_kendaraan = $request->kode_kendaraan;
        $keterangan = $request->keterangan;
        $tgl = date("Y-m-d  H:i:s");

        $cek_datacount = DB::table('pinjam_kendaraan')->get();
        if (count($cek_datacount) <= 0) {
            $kode_booking = $subbidang_kode . "-" . $tgl . "-" . "0001";
        } else if (count($cek_datacount) > 0) {
            $last_bookingcode = DB::select(DB::raw("SELECT MAX(right(kode_booking,4)) as kode_booking FROM pinjam_kendaraan"));
            $next_bookingcode = (int)$last_bookingcode + 1;
            $kode_booking = $subbidang_kode . "-" . $tgl . "-" . sprintf("%04s", $next_bookingcode + 1);

        }


        $cek_jml_pinjaman = DB::table('pinjam_kendaraan')
            ->where('kode_subbid', $subbidang_kode)
            ->get();
        /*mengganti string character*/
        $kode_booking = str_replace(array("-", " ", ":"), '', $kode_booking);


        $cek_pinjamkendaraan = DB::select(DB::raw("select * from pinjam_kendaraan 
where mulai_tgl='$mulai_tgl' and sampai_tgl='$sampai_tgl' and kode_kendaraan='$kode_kendaraan'"));
        //echo "<pre>"; print_r($cek_pinjamkendaraan); die;
        /*cekricek*/

        $jarakWaktu = abs(strtotime($mulai_tgl) - strtotime($sampai_tgl));
        $numberDays = $jarakWaktu / 86400;
        $numberDays = intval($numberDays) + 1;

        for ($i = 0; $i < $numberDays; $i++) {
            if ($i > 1) {
                $tgl_torent = date('Y-m-d', strtotime($mulai_tgl . '+' . $i . ' days'));

            } else if ($i <= 1) {
                $tgl_torent = date('Y-m-d', strtotime($mulai_tgl . '+' . $i . ' day'));
            }

            $cek_pivot_pinjamkendaraan = DB::select(DB::raw("select * from pivot_pinjamkendaraan 
where tgl_pinjam='$tgl_torent' and kode_kendaraan='$kode_kendaraan'"));
            //echo count($cek_pivot_pinjamkendaraan)."<br>";
            if (count($cek_pivot_pinjamkendaraan) > 0) {
                array_push($keep_data, (object)[
                    "status" => "Booked",
                    "tanggal" => $tgl_torent,
                    "kode_kendaraan" => $kode_kendaraan,
                ]);
            } else if (count($cek_pivot_pinjamkendaraan) <= 0) {
                array_push($keep_data, (object)[
                    "status" => "Available",
                    "tanggal" => $tgl_torent,
                    "kode_kendaraan" => $kode_kendaraan,
            ...
            //$cek_date = date($mulai_tgl);
        }

        $filterBy = "Booked";
        $new = array_filter($keep_data, function ($var) use ($filterBy) {
        ...
        //echo "<pre>"; print_r($keep_data); die;
        foreach ($keep_data as $index => $kd) {
            if ($kd->status == "Booked") {
                $get_plat = DB::table("kendaraan")->where("kode_kendaraan", $kd->kode_kendaraan)->first();
                array_push($filter_booked_date, (object)[
                    "status" => "fix_booked",
                    "tgl_booked" => $kd->tanggal,
                    "plat_kendaraan" => $get_plat->plat_kendaraan,
                    "kode_kendaraan" => $kd->kode_kendaraan,
                ]);
            } else if ($kd->status == "Available") {
                $get_plat = DB::table("kendaraan")->where("kode_kendaraan", $kd->kode_kendaraan)->first();
                array_push($filter_available_date, (object)[
        ...
        if ($simpan_indikator == "n") {
            foreach ($filter_booked_date as $ind => $dta) {
                $tgl_booked = $dta->tgl_booked;
                $plat_kendaraan = $dta->plat_kendaraan;
                $tanggal_gagal .= $tgl_booked . ", ";

            }
            $warning = "Gagal!!! Kendaraan dgn plat " . $plat_kendaraan . " telah dibooking pada " . $tanggal_gagal . " Mohon memilih range tanggal peminjaman yang lain";
            return redirect()->route('landingpage')->with(['warning' => $warning]);

        } else if ($simpan_indikator == "y") {

            try {
                /*simpan ke tabel pinjam kendaraan*/
                if ($request->hasFile('datadukung')) {

                    $tgl_lengkap = date('Y-m-d H:i:s');
                    $unique_code = explode(' ', $tgl_lengkap);
                    $unique_code_name = str_replace(':', '-', $unique_code[1]);
                    $datadukung = pathinfo($request->datadukung->getClientOriginalName(), PATHINFO_FILENAME) . $unique_code_name . '.' . $request->datadukung->getClientOriginalExtension();
                    $request->datadukung->storeAs('public/uploads/datadukung', $datadukung);
                    //echo $datadukung;
                } else {
                    $datadukung = "no data";
                }
                ...
                $data_tosave = [
                    ...
                    "perihal_pinjaman" => $perihal_pinjaman,
                    "mulai_tgl" => $mulai_tgl,
                    "sampai_tgl" => $sampai_tgl,
                    "penanggungjawab" => $penanggung_jawab,
                    "kode_subbid" => $subbidang_kode,
                    "kode_kendaraan" => $kode_kendaraan,
                    "datadukung" => json_encode($datadukung),
                    "status" => "Pengajuan Pinjaman",
                    "keterangan" => $keterangan,
                    "created_at" => date('Y-m-d H:i:s'),
                    "updated_at" => date('Y-m-d H:i:s'),
                ];
                $simpan = DB::table('pinjam_kendaraan')->insert($data_tosave);
                if ($simpan) {

                    $warning = "Pengajuan pinjaman berhasil dikirim";
                    /*simpan ke tabel pivot pinjam kendaraan*/
                    foreach ($filter_available_date as $idx => $dtx) {
                        $data_pivotkendaraan = [
                            "kode_kendaraan" => $dtx->kode_kendaraan,
                            "tgl_pinjam" => $dtx->tgl_booked,
                        ];
                        $simpan_tbl_pivotkendaraan = DB::table("pivot_pinjamkendaraan")->insert($data_pivotkendaraan);
                    }
                    return redirect()->route('landingpage')->with(['success' => 'Data Berhasil Disimpan dengan Kode Booking|' . $kode_booking]);
                } else {
                    return redirect()->route('landingpage')->with(['warning' => 'Data Gagal Disimpan']);
                }

            } catch (\Exception $e) {
                return redirect()->route('landingpage')->with(['warning' => 'Data Gagal Disimpan Bor, error : ' . $e]);
            }
    ...
    {
        $keep_data_ruangan = [];
        $filter_booked_date_ruangan = [];
        $filter_available_date_ruangan = [];
        $simpan_indikator_ruangan = "n";
        $tanggal_gagal_ruangan = "";
        ...
        $cek_datacount = DB::table('pinjam_ruangan')->get();
        if (count($cek_datacount) <= 0) {
            $kode_booking = $subbidang_kode . "-" . $tgl . "-" . "0001";
        } else if (count($cek_datacount) > 0) {
            $last_bookingcode = DB::select(DB::raw("SELECT MAX(right(kode_booking,4)) as kode_booking FROM pinjam_ruangan"));
            $next_bookingcode = (int)$last_bookingcode + 1;
            $kode_booking = $subbidang_kode . "-" . $tgl . "-" . sprintf("%04s", $next_bookingcode + 1);
        }
        $cek_jml_pinjaman = DB::table('pinjam_ruangan')
            ->where('kode_subbid', $subbidang_kode)
            ->get();
        $kode_booking = str_replace(array("-", " ", ":"), '', $kode_booking);

        $cek_pinjamruangan = DB::select(DB::raw("select * from pinjam_ruangan 
where mulai_tgl='$mulai_tgl' and sampai_tgl='$sampai_tgl' and kode_ruangan='$kode_ruangan'"));
        ...
        for ($i = 0; $i < $numberDays; $i++) {
            ...
            $cek_pivot_pinjamruangan = DB::select(DB::raw("select * from pivot_pinjamruangan 
where tgl_pinjam='$tgl_torent' and kode_ruangan='$kode_ruangan'"));

            if (count($cek_pivot_pinjamruangan) > 0) {
                array_push($keep_data_ruangan, (object)[
                    "status" => 'Booked',
                    "tanggal" => $tgl_torent,
                    "kode_ruangan" => $kode_ruangan,
                ]);
            } else if (count($cek_pivot_pinjamruangan) <= 0) {
                array_push($keep_data_ruangan, (object)[
                    "status" => 'Available',
                    "tanggal" => $tgl_torent,
                    "kode_ruangan" => $kode_ruangan,
        ...
        $filterBy = "Booked";
        $new_ruangan = array_filter($keep_data_ruangan, function ($var) use ($filterBy) {
            return ($var->status == $filterBy);
        });

        if (count($new_ruangan) > 0) {
            $simpan_indikator = "n";
        } else if (count($new_ruangan) <= 0) {
            $simpan_indikator = "y";
        }

        foreach ($keep_data_ruangan as $index => $kd_ruangan) {
            if ($kd_ruangan->status == "Booked") {
                $get_namaruangan = DB::table("ruangan")->where('kode_ruangan', $kd_ruangan->kode_ruangan)->first();
                array_push($filter_booked_date_ruangan, (object)[
                    "status" => 'fix_booked',
                    "tgl_booked" => $kd_ruangan->tanggal,
                    "nama_ruangan" => $get_namaruangan->name_ruangan,
                    "kode_ruangan" => $kd_ruangan->kode_ruangan,
                ]);
            } else if ($kd_ruangan->status == "Available") {
                $get_namaruangan = DB::table("ruangan")->where('kode_ruangan', $kd_ruangan->kode_ruangan)->first();
                array_push($filter_available_date_ruangan, (object)[
                    "status" => 'fix_available',
                    "tgl_booked" => $kd_ruangan->tanggal,
                    "nama_ruangan" => $get_namaruangan->name_ruangan,
                    "kode_ruangan" => $kd_ruangan->kode_ruangan,
        ...
        if ($simpan_indikator == "n") {
            foreach ($filter_booked_date_ruangan as $ind => $dta) {
                $tgl_booked = $dta->tgl_booked;
                $nama_ruangan = $dta->nama_ruangan;
                $tanggal_gagal_ruangan .= $tgl_booked . ", ";
            }
            $warning = "Gagal!!! Ruangan " . $nama_ruangan . " telah dibooking pada " . $tanggal_gagal_ruangan . " Mohon memilih range tanggal peminjaman yang lain";
            return redirect()->route('landingpage')->with(['warning' => $warning]);
        } else if ($simpan_indikator == "y") {
            //echo "simpan indikator: ".$simpan_indikator;
            try {
                if ($request->hasFile('datadukung')) {
                    $tgl_lengkap = date('Y-m-d H:i:s');
                    $unique_code = explode(' ', $tgl_lengkap);
                    $unique_code_name = str_replace(':', '-', $unique_code[1]);
                    $datadukung = pathinfo($request->datadukung->getClientOriginalName(), PATHINFO_FILENAME) . $unique_code_name . '.' . $request->datadukung->getClientOriginalExtension();
                    $request->datadukung->storeAs('public/uploads/datadukungruangan', $datadukung);
                } else {
                    $datadukung = "no data";
                }
                ...
                $simpan = DB::table("pinjam_ruangan")->insert($data_tosave);
                if ($simpan) {
                    $warning = "Pengajuan pinjaman ruangan berhasil dikirim";
                    /*simpan ke tabel pivot pinjam ruangan*/
                    foreach ($filter_available_date_ruangan as $idx => $dtx) {
                        $data_pivotruangan = [
                            "kode_ruangan" => $dtx->kode_ruangan,
                            "tgl_pinjam" => $dtx->tgl_booked,
                        ];
                        $simpan_tbl_pivotruangan = DB::table('pivot_pinjamruangan')->insert($data_pivotruangan);
                    }
                    return redirect()->route('landingpage')->with(['success' => 'Data Berhasil Disimpan dengan Kode Booking|' . $kode_booking]);
                } else {
                    return redirect()->route('landingpage')->with(['warning' => 'Data Gagal Disimpan']);
    ...
    public function formpinjamruangan()
    {
        $subbidangs = DB::table('subbidang')->get();
        $ruangans = DB::table('ruangan')->get();
        return view('pinjamsarpras.formpinjamruangan', compact('subbidangs', 'ruangans'));
    }
    ...
    {
        //echo "cek kode booking"; die;
        return view('pinjamsarpras.formcekkodebooking');
    ...
    public function showhistori(Request $request){
        ...
        //return $request;
        if($request->jenis_pinjaman=="pinjam_kendaraan"){
            $getdata = DB::table('pinjam_kendaraan')->where('kode_booking',$request->kode_booking)->get();
        } else if($request->jenis_pinjaman=="pinjam_ruangan"){
            $getdata = DB::table('pinjam_ruangan')->where('kode_booking',$request->kode_booking)->get();
        }

        //return $getdata;
        //echo count($getdata); die;

        return view('pinjamsarpras.gethistori', compact('getdata'));
    }

    public function tampilkankodebooking(Request $request)
    {
        $kode_booking = $request->kode_booking;
        //return $kode_booking;
        //dd($request);
        return view('pinjamsarpras.tampilkankodebooking', compact('kode_booking'));
    }
...
//            $url_media_nasional_tosave = [];
    ...
    public function rekapmediaexternal()
    {
        if (auth()->user()->roles == 'humas_satker') {
            abort(403);
        }
        //echo "rekapmediaexternal"; die;
        $satker = DB::table('satker')->orderBy('kode_satker')->get();
        return view('laporan.rekapmediaexternal', compact('satker'));
    }

    public function laporanberitamedia()
    {
        //echo "laporan berita by media broyy"; die;
        $namabulan = [
            "",
            "Januari",
            "Februari",
    ...
    public function cetakrekapmediaexternal(Request $request)
    {
        $namabulan = ["", "Januari", "Februari", "Maret", "April", "Mei", "Juni", "Juli", "Agustus", "September",
            "Oktober", "November", "Desember",
        ];
...
where kode_satker='$kode_satker' and length(youtube)>10 AND tgl_input between '$dari' and '$sampai'"));
        ...
        if ($request->jenis_media == 'media_lokal') {
            //echo "media lokal selected<br>";
            /*intervensi by jenis media (MEDIA LOKAL / MEDIA NASIONAL)*/
            ...
            if (isset($_POST['exportexcelrekapexternal'])) {
                //echo "media lokal excel selected";
                $time = date('d-M-Y H:i:s');
                header("Content-type: application/vnd-ms-excel");
                header("Content-Disposition: attachment; filename=RekapitulasiLinkBeritaMediaLokal_$satker_name$time.xls");
                return view('laporan.cetakrekapexternal_excel_medialokal', compact('satker_name', 'tgl_dari', 'namabulan', 'bulan_dari',
                    'tahun_dari', 'tgl_sampai', 'bulan_sampai', 'tahun_sampai', 'getberita', 'mediapartner'));
            }

            return view('laporan.cetakrekapexternal_medialokal', compact('tgl_dari', 'namabulan', 'bulan_dari',
                'tahun_dari', 'tgl_sampai', 'bulan_sampai', 'tahun_sampai', 'getberita', 'mediapartner'));
            ...
            if (isset($_POST['exportexcelrekapexternal'])) {
                //echo "media nasional excel selected"; die;
                $time = date('d-M-Y H:i:s');
                header("Content-type: application/vnd-ms-excel");
                header("Content-Disposition: attachment; filename=RekapitulasiLinkBeritaMediaNasional_$satker_name$time.xls");
                return view('laporan.cetakrekapexternal_excel_medianasional', compact('satker_name', 'tgl_dari', 'namabulan', 'bulan_dari',
                    'tahun_dari', 'tgl_sampai', 'bulan_sampai', 'tahun_sampai', 'getberita', 'mediapartner'));
            }
            return view('laporan.cetakrekapexternal_medianasional', compact('tgl_dari', 'namabulan', 'bulan_dari',
                'tahun_dari', 'tgl_sampai', 'bulan_sampai', 'tahun_sampai', 'getberita', 'mediapartner'));
        }
        //die;
    }
