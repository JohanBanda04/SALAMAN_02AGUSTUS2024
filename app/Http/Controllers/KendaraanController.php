...
class KendaraanController extends Controller
{
    ...
    public function kendaraandata(Request $request)
    {
        $default_jeniskendaraan = "";

        /*bisa di intervensi dari sini utk tiap roles*/
        $kendaraans = DB::table('kendaraan');
        if (!empty($request->jenis_kendaraan_cari)) {
            $default_jeniskendaraan = $request->jenis_kendaraan_cari;
            $kendaraans->where('jenis_kendaraan', $default_jeniskendaraan);
        }

        if (!empty($request->nama_kendaraan_cari)) {
            $kendaraans->where('name_kendaraan', 'like', '%' . $request->nama_kendaraan_cari . '%');
        }

        $kendaraans->orderBy("created_at", "desc");
        $dtKendaraans = $kendaraans->paginate(10);

        $results = DB::select(DB::raw("SELECT MAX(right(kode_kendaraan,2)) as kode FROM kendaraan"));

//        $results = DB::table('kendaraan')
//            ->whereRaw("MAX(right(kode_kendaraan,2))")
//            ->get();
        $kode_new = $results[0]->kode + 1;
        //dd($kode_new);
        //echo $kode_new; die;
        $kode_kendaraan = "KWK-NTB-0" . $kode_new;
        //dd($kode_kendaraan);
        return view('kendaraan.index', compact('dtKendaraans', 'kode_kendaraan', 'default_jeniskendaraan'));
    }
    ...
    public function kendaraanstore(Request $request)
    {
        ...
        if ($request->hasFile('foto_kendaraan')) {
            //echo "ada";
            $foto_kendaraan = $kode_kendaraan . "." . $request->file('foto_kendaraan')->getClientOriginalExtension();
        } else {
            //echo "tdk ada";
            $foto_kendaraan = null;
        }

        $cek_kendaraan = DB::table('kendaraan')
            ->where('name_kendaraan', $nama_kendaraan)
            ->orWhere('kode_kendaraan', $kode_kendaraan)
            ->orWhere('plat_kendaraan', $plat_kendaraan);

        if ($cek_kendaraan->count() > 0) {
            //echo "data sudah ada";
            return redirect()->route('kendaraandata')->with(['warning' => 'Gagal Menyimpan!! Nama Kendaraan Sudah Ada / Plat Kendaraan Telah Terdaftar']);
        } else {
            //echo "data belum ada";
            try {
                $data = [
                    "kode_kendaraan" => $kode_kendaraan,
                    "name_kendaraan" => $nama_kendaraan,
                ...
                if($simpan){
                    if ($request->hasFile('foto_kendaraan')) {
                        $folderPath = "public/uploads/salaman";
                        $request->file('foto_kendaraan')->storeAs($folderPath, $foto_kendaraan);
                    }
                    return redirect()->route('kendaraandata')->with(['success' => 'Data Berhasil Disimpan']);
                } else {
                    return redirect()->route('kendaraandata')->with(['warning' => 'Data Gagal Disimpan']);
                }
            } catch (Exception $e){
                return redirect()->route('kendaraandata')->with(['warning' => 'Data Gagal Disimpan Bor, error: '.$e]);
            }
        }
    }
